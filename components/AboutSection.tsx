        ...
        <div className="flex flex-col lg:flex-row justify-between gap-8 mt-16 lg:mt-24">
          <div className="space-y-4 flex-1">
            {MISSION_FEATURES.map((f, i) => (
              <div key={i} className="liquid-glass rounded-[20px] p-5 hover:bg-white/5 transition-all duration-300">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-grotesk text-base uppercase text-cream tracking-wide">{f.title}</h3>
                    <p className="font-mono text-xs text-cream/60 mt-1 leading-relaxed">{f.desc}</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {f.tags.map((t) => (
                        <span key={t} className="px-2 py-0.5 text-[10px] font-mono text-cream/30 border border-cream/5 rounded-md uppercase tracking-wider">
                          {t}
                        </span>
                      ))}
                    </div>
